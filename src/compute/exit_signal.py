...
def exit_flag(row: dict | pd.Series, max_trend_days: int | None = None) -> dict:
    data = dict(row)
    sector = str(data.get("sector", "当前行业") or "当前行业")
    structural_reasons = []

    if bool(data.get("below_box_low", False)):
        structural_reasons.append(f"{sector} 跌破箱体下沿")

    if "price_above_ma60" in data and data.get("price_above_ma60") is False:
        structural_reasons.append(f"{sector} 跌破 MA60")

    cum_inflow_20d = _optional_float(data.get("cum_inflow_20d"))
    if cum_inflow_20d is not None and cum_inflow_20d < 0:
        structural_reasons.append(f"{sector} 多周资金转出")

    midterm = _optional_float(data.get("midterm_trend"))
    if midterm is not None and midterm < 0:
        structural_reasons.append(f"{sector} 中期趋势转弱")
    ...
    pullback = _optional_float(data.get("pullback_from_high"))
    if (
        pullback is not None
        and pullback <= 0.10
        and (midterm is None or midterm >= 0)
        and data.get("price_above_ma60", True) is not False
        and (cum_inflow_20d is None or cum_inflow_20d >= 0)
    ):
        return {
            "exit": False,
            "level": "hold",
            "reason": f"{sector} 中期结构完好，10%内回撤属于正常波动。",
        }

    watch_reasons = []
    if bool(data.get("turning_point", False)):
        watch_reasons.append(f"{sector} 出现拐点，考虑减仓观察")

    trend_days = int(data.get("trend_days", 0) or 0)
    if max_trend_days is not None and trend_days >= max_trend_days:
        watch_reasons.append(f"{sector} 趋势已持续 {trend_days} 天，属于超期提醒")
    ...
    if pct_chg > 0 and main_net_inflow < 0:
        watch_reasons.append(f"{sector} 价格仍涨但主力资金转弱，出现量价背离")
    ...
    return {"exit": False, "level": "hold", "reason": ""}
